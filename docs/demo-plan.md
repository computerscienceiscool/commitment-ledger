...
- `make demo-config` writes the same shape to `config/repos.demo.json` if you need only the config file
- `make demo-setup` is the normal end-to-end helper because it initializes repos, seeds TODO content, and writes the demo config together

The rendered walkthrough asset for this scenario lives at `docs/demo-video.mp4`
and is generated by `tools/render_demo_video.py`.
