# Commitment Ledger Protocol: commitment-assessment-v2
...
- `basis` lists supporting evidence artifact CIDs for that same commitment.
- Assessment is local and explicit; there is no global automatic verdict.
- Local v0.2 assessment flow only allows assessment from `open` or `expired_unassessed`; it does not silently rewrite already-finalized commitments.
